...
A multi-tenant test management platform (requirements, test cases, execution,
defects, traceability, reporting) comparable in capability to Zephyr. See
`blueprint/context/project-overview.md` for the full picture.
...
Both apps are scaffolded and these commands are verified (feature 1a). Local
infra (Postgres, Redis, Keycloak) runs via Docker Compose.

### Infra

- Start: `docker-compose up -d` (Postgres on `:5432`, Redis on `:6379`, Keycloak on `:8081`)
...
- Dev server: `./mvnw spring-boot:run` (http://localhost:8080; requires
  Postgres running - see Infra above)
- Build: `./mvnw clean package -DskipTests`
- Health check: `curl http://localhost:8080/actuator/health`

Testing is opt-in and not yet configured for either app - no `test` command is
declared here, so the testing gate (`coding-standards.md`) stays off. Run
`/tests` or `$tests` when you want to add a unit test runner and turn the gate
on.
